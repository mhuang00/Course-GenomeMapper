...
Furthermore, Googletest needs to be added via CMake's FetchContent module, accompanied with one unit tests which has an always true assertion (which is used here just to test Googletest integration). Every piece of code following afterwards should have its own unit tests. In order to test the code after each commit on various compilers (choose several gcc and clang versions) and systems (choose Ubuntu and macOS), create a GitHub Actions workflow choosing several compiler versions for gcc/clang and both Ubuntu and macOS systems.

Write the usage message and compilation instructions inside the README located in the root of this project. Add a [badge](https://docs.github.com/en/actions/managing-workflow-runs/adding-a-workflow-status-badge) denoting the result of GitHub Actions.
